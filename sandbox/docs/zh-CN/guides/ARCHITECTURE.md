# HTTP Service 项目架构总结

> **相关文档**:
> - 本文档：系统整体架构概览
> - [后端开发详细指南](../development/BACKEND_DEVELOPMENT.md)：完整的后端开发教程和示例
> - [使用指南](USAGE_GUIDE.md)：Sandbox 使用与启动方式

## 📋 目录

1. [整体架构](#整体架构)
2. [核心组件](#核心组件)
3. [数据流](#数据流)
4. [生命周期管理](#生命周期管理)
5. [Session 管理机制](#session-管理机制)
6. [工具注册机制](#工具注册机制)
7. [后端扩展系统](#后端扩展系统)
8. [配置系统](#配置系统)
9. [API 接口](#api-接口)

---

## 🏗️ 整体架构

```
┌─────────────────────────────────────────────────────────────────────────┐
│                          用户层 (User Layer)                             │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ┌──────────────────────────────────────────────────────────────────┐   │
│  │                    Sandbox (Facade)                              │   │
│  │  - 统一入口，简化用户交互                                          │   │
│  │  - 自动服务器启动/检测                                             │   │
│  │  - 支持同步/异步双模式                                             │   │
│  └──────────────────────────────────────────────────────────────────┘   │
│                              │                                           │
│                              ▼                                           │
│  ┌──────────────────────────────────────────────────────────────────┐   │
│  │              HTTPServiceClient (HTTP Client)                     │   │
│  │  - HTTP 请求封装                                                 │   │
│  │  - Worker ID 管理                                                │   │
│  │  - 自动重试/错误处理                                             │   │
│  └──────────────────────────────────────────────────────────────────┘   │
│                              │                                           │
│                              │ HTTP/JSON                                  │
│                              ▼                                           │
└─────────────────────────────────────────────────────────────────────────┘
                              │
                              │
┌─────────────────────────────────────────────────────────────────────────┐
│                        服务层 (Server Layer)                             │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ┌──────────────────────────────────────────────────────────────────┐   │
│  │              HTTPServiceServer (FastAPI)                          │   │
│  │  - 持有工具数据结构（三层映射）                                    │   │
│  │  - 持有 Backend 实例                                              │   │
│  │  - 反射扫描 @tool 标记并注册                                       │   │
│  │  - HTTP 路由 (routes.py)                                          │   │
│  └──────────────────────────────────────────────────────────────────┘   │
│                              │                                           │
│        ┌─────────────────────┴─────────────────────┐                   │
│        ▼                                             ▼                   │
│  ┌──────────────────┐                    ┌──────────────────┐          │
│  │  ToolExecutor    │                    │ ResourceRouter   │          │
│  │  - 工具执行      │                    │  - Session 管理   │          │
│  │  - 路由解析      │                    │  - 资源路由       │          │
│  │  - 参数注入      │                    │  - 临时 Session   │          │
│  └──────────────────┘                    └──────────────────┘          │
│                              │                                           │
│                              ▼                                           │
│  ┌──────────────────────────────────────────────────────────────────┐   │
│  │                    Backend System                                │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐          │   │
│  │  │ VMBackend    │  │ RAGBackend   │  │ API Tools    │          │   │
│  │  │ (Session)    │  │ (共享资源)   │  │ (轻量级)     │          │   │
│  │  └──────────────┘  └──────────────┘  └──────────────┘          │   │
│  └──────────────────────────────────────────────────────────────────┘   │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## 🔧 核心组件

### 1. Sandbox (用户门面)

**位置**: `sandbox/sandbox.py`

**职责**:
- 提供统一的用户接口
- 封装 `HTTPServiceClient`
- 自动服务器启动/检测
- Session 批量创建管理
- 支持同步/异步双模式

**关键方法**:
```python
- start()                    # 启动服务器（触发 Backend.warmup）
- warmup()                   # 显式预热后端资源
- get_warmup_status()        # 获取预热状态
- create_session()           # 创建 Session（触发 Backend.initialize）
- execute()                  # 执行工具（可自动创建临时 Session，自动预热）
- destroy_session()          # 销毁 Session（触发 Backend.cleanup）
- close()                    # 关闭连接
- shutdown_server()          # 关闭服务器（触发 Backend.shutdown）
```

### 2. HTTPServiceServer (HTTP 服务器)

**位置**: `sandbox/server/app.py`

**职责**:
- **持有工具数据结构**（三层映射）
- **持有 Backend 实例**
- 反射扫描 @tool 标记并注册
- 调度请求到对应的工具函数
- 管理后端生命周期

**数据结构**:
```python
# 三层工具映射（Server 持有）
self._tools: Dict[str, Callable]           # 完整名称 → 函数
self._tool_name_index: Dict[str, List[str]] # 简单名称 → 完整名称列表
self._tool_resource_types: Dict[str, str]   # 完整名称 → 资源类型

# Backend 实例持有
self._backends: Dict[str, Backend]          # 资源类型 → Backend 实例
```

### 3. ToolExecutor (工具执行器)

**位置**: `sandbox/server/core/tool_executor.py`

**职责**:
- 工具执行逻辑
- 资源类型前缀解析
- Session 自动注入
- **临时 Session 管理**（用完即销毁）

**执行流程**:
```python
# 1. 解析资源类型前缀
action = "vm:screenshot"
  → resource_type = "vm"
  → tool_name = "screenshot"

# 2. 获取/创建 Session
if resource_type:
    existing = router.get_session(worker_id, resource_type)
    if existing:
        session_info = existing
        is_temporary = False
    else:
        session_info = router.create_session(...)
        is_temporary = True  # 临时 Session

# 3. 执行工具
result = await func(**params)

# 4. 临时 Session 自动销毁
if is_temporary:
    await router.destroy_session(...)
```

### 4. ResourceRouter (资源路由器)

**位置**: `sandbox/server/core/resource_router.py`

**职责**:
- Session 生命周期管理
- Worker 资源隔离
- 注册 Backend 的 initialize/cleanup 函数
- 过期 Session 清理

---

## 🔄 数据流

### 执行命令流程

```
用户代码
  │
  │ sandbox.execute("vm:screenshot", {})
  ▼
Sandbox.execute()
  │
  │ await client.execute("vm:screenshot", {})
  ▼
HTTPServiceClient.execute()
  │
  │ POST /execute
  │ Body: {"worker_id": "...", "action": "vm:screenshot", "params": {}}
  ▼
HTTPServiceServer (routes.py)
  │
  │ await server.execute(action, params, worker_id)
  ▼
ToolExecutor.execute()
  │
  │ 1. 解析: "vm:screenshot" → resource_type="vm"
  │ 2. 查找工具: func = _tools["vm:screenshot"]
  │ 3. 获取/创建 Session（可能是临时的）
  │ 4. 注入参数: params["session_info"] = session_info
  │ 5. 执行: result = await func(**params)
  │ 6. 如果临时 Session → 自动销毁
  ▼
返回结果
  │
  │ {"success": True, "data": {...}, "temporary_session": True/False}
  ▼
用户代码
```

---

## 📅 生命周期管理

### Backend 生命周期方法

| 方法 | 调用时机 | 触发者 | 作用 |
|------|---------|--------|------|
| `warmup()` | 服务器启动或显式调用 | `sandbox.warmup()` 或执行工具时自动触发 | 预热共享资源（资源池、模型加载） |
| `initialize()` | 创建 Session | `create_session()` 或自动创建 | 分配 worker 专属资源 |
| `cleanup()` | 销毁 Session | `destroy_session()` 或自动销毁 | 回收 worker 资源 |
| `shutdown()` | 服务器关闭 | `shutdown_server()` | 释放所有共享资源 |

### 自动预热机制

执行工具时，如果后端尚未预热，系统会自动触发预热：

```python
# 执行流程（自动预热）
await sandbox.execute("rag:search", {"query": "test"})
# 1. 检查 RAG 后端是否已预热
# 2. 未预热 → 自动调用 backend.warmup()
# 3. 执行工具
```

也可以显式预热以减少首次调用延迟：

```python
# 显式预热
await sandbox.warmup(["rag", "vm"])  # 预热指定后端
await sandbox.warmup()                # 预热所有后端

# 获取预热状态
status = await sandbox.get_warmup_status()
# {"backends": {"rag": {"loaded": True, "warmed_up": True}, ...}}
```

### 生命周期时序图

```
┌──────────────────────────────────────────────────────────────────┐
│                    完整生命周期时序                               │
├──────────────────────────────────────────────────────────────────┤
│                                                                  │
│   sandbox.start()                                                │
│       │                                                          │
│       └── server.load_backend(backend)                           │
│               │                                                  │
│               └── scan_and_register(backend)  ← 注册工具         │
│                                                                  │
│   sandbox.warmup(["rag", "vm"])     ← 显式预热（可选）           │
│       │                                                          │
│       └── await backend.warmup()                                 │
│                                                                  │
│   ════════════════════════════════════════════════════════════  │
│                                                                  │
│   sandbox.execute("rag:search", {})                              │
│       │                                                          │
│       ├── (自动) await backend.warmup()  ← 若后端未预热          │
│       └── 执行工具函数                                           │
│                                                                  │
│   sandbox.create_session("vm", config)                           │
│       │                                                          │
│       └── await backend.initialize(worker_id, config)  ← ⭐      │
│                                                                  │
│   sandbox.execute("vm:screenshot", {})                           │
│       │                                                          │
│       └── 使用现有 Session 执行                                  │
│                                                                  │
│   sandbox.destroy_session("vm")                                  │
│       │                                                          │
│       └── await backend.cleanup(worker_id, session_info)  ← ⭐   │
│                                                                  │
│   ════════════════════════════════════════════════════════════  │
│                                                                  │
│   sandbox.close()                                                │
│       │                                                          │
│       └── 只关闭客户端连接，服务器继续运行                       │
│                                                                  │
│   sandbox.shutdown_server()                                      │
│       │                                                          │
│       └── await backend.shutdown()  ← ⭐ 释放 GPU 等资源         │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```

### Sandbox API 与 Backend 生命周期对应

| Sandbox API | Backend 方法 | 说明 |
|-------------|-------------|------|
| `start()` | `warmup()` (可选) | 启动服务器，可选预热后端 |
| `warmup(resources)` | `warmup()` | 显式预热后端资源 |
| `get_warmup_status()` | - | 获取预热状态 |
| `create_session(type, config)` | `initialize(worker_id, config)` | 显式创建 Session |
| `execute(action, params)` | `warmup()` (自动) + 工具函数 + 可能的 `initialize`/`cleanup` | 执行（自动预热，可自动创建临时 Session） |
| `destroy_session(type)` | `cleanup(worker_id, session_info)` | 显式销毁 Session |
| `close()` | - | 关闭客户端连接（服务器继续运行） |
| `shutdown_server()` | `shutdown()` | 关闭服务器（释放 GPU 等资源） |

---

## 📦 Session 管理机制

### 两种 Session 模式

| 模式 | 创建方式 | 执行后 | 适用场景 |
|------|---------|--------|---------|
| **显式创建（复用模式）** | `create_session()` | 保持存活 | 多次操作同一资源 |
| **自动创建（临时模式）** | 执行时自动 | 立即销毁 | 单次操作 |

### 显式创建 Session

```python
async with Sandbox() as sandbox:
    # 显式创建 - 会复用
    await sandbox.create_session("vm", {
        "screen_size": [1920, 1080],
        "custom_name": "my_vm"
    })
    
    await sandbox.execute("vm:screenshot", {})  # 复用
    await sandbox.execute("vm:click", {"x": 100})  # 复用
    
    await sandbox.destroy_session("vm")
```

### 自动创建临时 Session

```python
async with Sandbox() as sandbox:
    # 直接执行，自动创建临时 Session
    await sandbox.execute("vm:screenshot", {})
    # Session 已自动销毁
```

### 返回结果字段

```json
{
    "success": true,
    "data": {...},
    "tool": "screenshot",
    "resource_type": "vm",
    "session_id": "xxx",
    "temporary_session": true
}
```

> 说明：`create_session` 的配置会在 ResourceRouter 层与默认配置合并，
> 用户传入字段会覆盖默认值。

---

## 🔌 工具注册机制

### 两种注册方式

| 方式 | 装饰器 | 适用于 | 注册时机 |
|------|--------|--------|---------|
| **重量级后端** | `@tool` | Backend 类方法 | `load_backend()` 反射扫描 |
| **轻量级工具** | `@register_api_tool` | `BaseApiTool` 实例（函数式兼容） | 模块导入时 |

### 重量级后端工具

```python
# 使用 @tool 装饰器标记
class VMBackend(Backend):
    @tool("vm:screenshot")
    async def screenshot(self, session_info: Dict) -> Dict:
        vm = session_info["data"]["vm"]
        return {"image": await vm.capture()}

# Server 加载时反射扫描
server.load_backend(VMBackend())
# → scan_and_register() 发现 @tool 标记
# → 注册到 _tools["vm:screenshot"]
```

### 轻量级 API 工具

```python
# 推荐：BaseApiTool 实例 + register_api_tool
class SearchTool(BaseApiTool):
    async def execute(self, query: str, **kwargs) -> Dict:
        api_key = self.get_config("api_key")
        return {"results": [...]}

register_api_tool("web:search", config_key="websearch")(SearchTool())

# 模块导入时自动注册到全局 _API_TOOLS
# ConfigLoader._load_api_tools() 将其注册到 Server
```

### 工具数据结构

```python
# Server 持有的三层映射
_tools = {
    "web:search": <callable object>,    # 轻量级工具
    "vm:screenshot": <bound method>,    # 重量级工具
}

_tool_name_index = {
    "search": ["web:search"],
    "screenshot": ["vm:screenshot"],
}

_tool_resource_types = {
    "vm:screenshot": "vm",
    # "web:search" 不在这里（无 resource_type）
}
```

---

## 🎨 后端扩展系统

### Backend 统一基类

```python
class Backend(ABC):
    """
    后端基类 - 所有方法都是可选的
    """
    name: str           # 后端名称
    description: str    # 描述
    version: str        # 版本
    
    # 全局生命周期（可选）
    async def warmup(self) -> None: pass
    async def shutdown(self) -> None: pass
    
    # Session 生命周期（可选）
    async def initialize(self, worker_id, config) -> Dict: ...
    async def cleanup(self, worker_id, session_info) -> None: ...
```

### 三种 Backend 类型

| 类型 | 实现的方法 | 适用场景 | 示例 |
|------|-----------|---------|------|
| **共享资源后端** | `warmup()`, `shutdown()` | 预热模型、连接池 | RAG |
| **Session 资源后端** | `initialize()`, `cleanup()` | 每 worker 独立实例 | VM, Bash |
| **混合后端** | 全部四个 | 共享 + Session | Browser |

### 存放位置

```
backends/
├── base.py              # Backend 基类
├── resources/           # 重量级后端
│   ├── vm.py            # Session 后端（VM 桌面自动化）
│   ├── rag.py           # 共享资源后端（RAG 检索）
│   ├── bash.py          # Session 后端（Bash 终端）
│   ├── browser.py       # 混合后端（浏览器自动化）
│   └── code_executor.py # Session 后端（代码执行）
└── tools/               # 轻量级 API 工具
    ├── __init__.py      # @register_api_tool
    └── websearch.py     # WebSearch 工具
```

---

## ⚙️ 配置系统

### 配置结构

```json
{
  "server": {
    "title": "Sandbox HTTP Service",
    "session_ttl": 300
  },
  
  "resources": {
    "vm": {
      "enabled": true,
      "backend_class": "sandbox.server.backends.resources.vm.VMBackend",
      "config": {"pool_size": 10}
    }
  },
  
  "apis": {
    "websearch": {
      "serper_api_key": "${SERPER_API_KEY}",
      "max_results": 10
    }
  }
}
```

### 环境变量支持

- `${VAR}` - 必需的环境变量
- `${VAR:-default}` - 带默认值

---

## 🔌 API 接口

### 核心端点

| 端点 | 方法 | 说明 |
|------|------|------|
| `/execute` | POST | 执行工具 |
| `/execute/batch` | POST | 批量执行 |
| `/session/create` | POST | 创建 Session |
| `/session/destroy` | POST | 销毁 Session |
| `/session/list` | POST | 列出 Session |
| `/tools` | GET | 列出工具 |
| `/health` | GET | 健康检查 |

### 执行响应

```json
{
  "success": true,
  "data": {"image": "..."},
  "tool": "screenshot",
  "resource_type": "vm",
  "session_id": "vm_xxx_001",
  "temporary_session": false,
  "execution_time_ms": 15.5
}
```

---

## 🎯 关键设计原则

### 1. 分层解耦

- **Server**：持有数据结构，调用高层接口
- **Backend**：隐藏复杂实现（资源池、模型等）
- **ToolExecutor**：纯执行逻辑

### 2. 开放接口，隐藏实现

```
Server 只知道:
├── backend.warmup()      → 预热
├── backend.initialize()  → 创建 Session
├── backend.cleanup()     → 销毁 Session
└── backend.shutdown()    → 关闭

Server 不知道:
├── 资源池如何实现
├── VM 是新建还是复用
├── 模型如何加载
└── 连接如何管理
```

### 3. 灵活的 Session 管理

- 显式创建：适合多次操作
- 自动创建：适合单次操作
- 临时 Session：用完即销毁

### 4. 统一的工具注册

- 重量级：`@tool` + 反射扫描
- 轻量级：`@register_api_tool` + `BaseApiTool` 配置注入

**快速决策**：需要长连接或持久状态？→ Backend；否则 → APITool

详细决策树请参考 [工具注册指南](../doc/REGISTRATION_GUIDE.md#-决策树选择-backend-还是-apitool)。

### 5. CI/CD 验证

由于使用动态类加载，建议在 CI/CD 中启用配置预检：

```bash
python -m sandbox server --config configs/profiles/production.json --validate
```

---

## 📊 项目结构

```
sandbox/
├── __init__.py              # 模块导出
├── sandbox.py               # 用户门面类
├── client.py                # HTTP 客户端
│
├── server/                  # 服务器模块
│   ├── app.py               # HTTPServiceServer（持有数据结构）
│   ├── routes.py            # HTTP 路由定义
│   ├── config_loader.py     # 配置加载器
│   │
│   ├── core/                # 核心组件
│   │   ├── decorators.py    # @tool 装饰器
│   │   ├── tool_executor.py # 工具执行器
│   │   └── resource_router.py # 资源路由器
│   │
│   └── backends/            # 后端实现
│       ├── base.py          # Backend 基类
│       ├── resources/       # 重量级后端（vm, rag, bash, browser, code_executor）
│       └── tools/           # 轻量级 API 工具（websearch）
│
├── configs/                 # 配置文件
└── USAGE_GUIDE.md           # 使用指南
```
