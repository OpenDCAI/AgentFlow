...
在项目根目录的 `configs/sandbox-server/` 下创建一个配置文件。建议先参考
`configs/sandbox-server/web_config.json`，再按需新增你的配置文件（例如 `my_api_sandbox.json`）。
...
可先参考现有实现 `sandbox/server/backends/tools/websearch.py`，然后在
`sandbox/server/backends/tools/` 下新建你的工具文件（例如 `my_api_tool.py`）。
