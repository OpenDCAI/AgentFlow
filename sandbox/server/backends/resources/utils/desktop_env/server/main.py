...
recording_path = "/home/user/recording.mp4"  # 改用 /home/user 避免 /tmp 空间限制
recording_start_time = None  # 诊断用：记录开始时间
...
recording_start_time = None  # 记录开始时间，用于诊断
...
def start_recording():
    ...
    # 禁用屏幕保护和电源管理，防止长时间录屏时屏幕休眠
    try:
        subprocess.run(['xset', 's', 'off'], check=False, capture_output=True)      # 禁用屏幕保护
        subprocess.run(['xset', '-dpms'], check=False, capture_output=True)         # 禁用 DPMS 电源管理
        subprocess.run(['xset', 's', 'noblank'], check=False, capture_output=True)  # 禁用屏幕空白
        logger.info("[RECORDING] Disabled screen saver and DPMS for long recording")
    except Exception as e:
        logger.warning(f"[RECORDING] Failed to disable screen saver: {e}")
    ...
    try:
        ...
        logger.info(f"[RECORDING] ulimit FSIZE: soft={soft}, hard={hard} (-1 means unlimited)")
        # 如果有限制，尝试解除
        if soft != resource.RLIM_INFINITY:
            resource.setrlimit(resource.RLIMIT_FSIZE, (resource.RLIM_INFINITY, hard))
            logger.info("[RECORDING] Removed file size limit")
    except Exception as e:
        logger.warning(f"[RECORDING] Could not check/set ulimit: {e}")

    # 使用 ultrafast preset + zerolatency 减少编码缓冲，-g 30 强制每秒输出关键帧
    start_command = f"ffmpeg -y -f x11grab -draw_mouse 1 -s {screen_width}x{screen_height} -i :0.0 -c:v libx264 -preset ultrafast -tune zerolatency -g 30 -r 30 {recording_path}"
    logger.info(f"[RECORDING] Starting ffmpeg: {start_command}")
    ...
    try:
        ...
        logger.info(f"[RECORDING DIAG] ffmpeg started successfully. PID={recording_process.pid}, start_time={recording_start_time}")
...
def end_recording():
    ...
    # 诊断：检查 ffmpeg 进程是否已经退出
    if not recording_process:
        logger.error("[RECORDING DIAG] end_recording called but recording_process is None!")
        return jsonify({'status': 'error', 'message': 'No recording in progress to stop.'}), 400
    
    poll_result = recording_process.poll()
    if poll_result is not None:
        # ffmpeg 进程已经退出！记录详细诊断信息
        ...
        logger.error(f"[RECORDING DIAG] ffmpeg ALREADY EXITED before end_recording was called!")
        logger.error(f"[RECORDING DIAG] Return code: {poll_result}")
        logger.error(f"[RECORDING DIAG] Recording duration (elapsed): {elapsed:.2f} seconds")
        logger.error(f"[RECORDING DIAG] File size: {file_size} bytes")
        logger.error(f"[RECORDING DIAG] ffmpeg stderr: {stderr_output}")
        
        recording_process = None
        recording_start_time = None
        
        # 即使 ffmpeg 提前退出，如果文件存在且非空，仍然返回文件
        if os.path.exists(recording_path) and file_size > 0:
            logger.warning(f"[RECORDING DIAG] Returning partial recording file ({file_size} bytes)")
            return send_file(recording_path, as_attachment=True)
        
        return jsonify({'status': 'error', 'message': f'ffmpeg exited prematurely. Return code: {poll_result}, stderr: {stderr_output}'}), 400

    # ffmpeg 还在运行，正常停止
    elapsed = time.time() - recording_start_time if recording_start_time else 0
    logger.info(f"[RECORDING DIAG] Stopping ffmpeg after {elapsed:.2f} seconds. PID={recording_process.pid}")
    ...
    if os.path.exists(recording_path) and os.path.getsize(recording_path) > 0:
        file_size = os.path.getsize(recording_path)
        logger.info(f"[RECORDING DIAG] Recording completed successfully. File size: {file_size} bytes")
        return send_file(recording_path, as_attachment=True)
    else:
...
def recording_status():
    ...
    if os.path.exists(recording_path):
        status_info['file_size'] = os.path.getsize(recording_path)
    
    if recording_process:
        status_info['recording_active'] = True
        status_info['ffmpeg_pid'] = recording_process.pid
        poll_result = recording_process.poll()
        
        if poll_result is None:
            status_info['ffmpeg_running'] = True
            # ffmpeg 还在运行，获取进程 IO 状态
            try:
                with open(f'/proc/{recording_process.pid}/io', 'r') as f:
                    io_stats = {}
                    for line in f:
                        key, val = line.strip().split(': ')
                        io_stats[key] = int(val)
                    status_info['write_bytes'] = io_stats.get('write_bytes', 0)
                    status_info['wchar'] = io_stats.get('wchar', 0)
            except Exception as e:
                status_info['io_error'] = str(e)
            
            # 非阻塞读取 ffmpeg stderr（检查是否有错误输出）
            try:
                import select
                if recording_process.stderr:
                    # 使用 select 检查是否有数据可读
                    readable, _, _ = select.select([recording_process.stderr], [], [], 0)
                    if readable:
                        # 非阻塞读取可用的数据
                        import fcntl
                        fd = recording_process.stderr.fileno()
                        flags = fcntl.fcntl(fd, fcntl.F_GETFL)
                        fcntl.fcntl(fd, fcntl.F_SETFL, flags | os.O_NONBLOCK)
                        try:
                            stderr_data = recording_process.stderr.read(4096)
                            if stderr_data:
                                status_info['ffmpeg_stderr_live'] = stderr_data[:1000]
                        except:
                            pass
                        finally:
                            fcntl.fcntl(fd, fcntl.F_SETFL, flags)
            except Exception as e:
                status_info['stderr_read_error'] = str(e)
        else:
            status_info['ffmpeg_running'] = False
            status_info['ffmpeg_return_code'] = poll_result
            # ffmpeg 已退出，尝试读取 stderr
            try:
                stderr = recording_process.stderr.read()
                if stderr:
                    status_info['ffmpeg_stderr'] = stderr[:1000]  # 限制长度
            except:
                pass
        
        if recording_start_time:
            status_info['elapsed_seconds'] = round(time.time() - recording_start_time, 2)
    
    # 添加磁盘空间诊断
    try:
        import shutil
        recording_dir = os.path.dirname(recording_path) or '/home/user'
        disk_usage = shutil.disk_usage(recording_dir)
        status_info['disk_total_mb'] = round(disk_usage.total / (1024*1024), 1)
        status_info['disk_used_mb'] = round(disk_usage.used / (1024*1024), 1)
        status_info['disk_free_mb'] = round(disk_usage.free / (1024*1024), 1)
    except Exception as e:
        status_info['disk_error'] = str(e)
        ...
        with open('/proc/meminfo', 'r') as f:
            ...
            for line in meminfo.split('\n'):
                if line.startswith('MemTotal:'):
                    status_info['mem_total_mb'] = int(line.split()[1]) // 1024
                elif line.startswith('MemAvailable:'):
                    status_info['mem_available_mb'] = int(line.split()[1]) // 1024
    except:
        pass
    ...
    try:
        import resource
        soft, hard = resource.getrlimit(resource.RLIMIT_FSIZE)
        status_info['ulimit_fsize_soft'] = soft if soft != resource.RLIM_INFINITY else 'unlimited'
        status_info['ulimit_fsize_hard'] = hard if hard != resource.RLIM_INFINITY else 'unlimited'
    except Exception as e:
        status_info['ulimit_error'] = str(e)

    # 检查文件系统类型
    try:
        result = subprocess.run(['df', '-T', recording_path], capture_output=True, text=True)
        if result.returncode == 0:
            lines = result.stdout.strip().split('\n')
            if len(lines) >= 2:
                parts = lines[1].split()
                if len(parts) >= 2:
                    status_info['fs_type'] = parts[1]  # 文件系统类型
    except:
        pass
    
    logger.info(f"[RECORDING DIAG] Status check: {status_info}")
