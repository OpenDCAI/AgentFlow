...
logger = logging.getLogger("desktopenv.metrics")
...
try:
    ...
    logger.warning("VLC metrics unavailable (missing optional deps): %s", exc)

    def _vlc_missing(*_args, **_kwargs):
        raise ImportError(
            "VLC metrics require optional dependencies (e.g. scipy). "
            "Install them to use VLC-related evaluators."
        )

    is_vlc_playing = _vlc_missing
    is_vlc_recordings_folder = _vlc_missing
    is_vlc_fullscreen = _vlc_missing
    compare_images = _vlc_missing
    compare_audios = _vlc_missing
    compare_videos = _vlc_missing
    check_qt_bgcone = _vlc_missing
    check_one_instance_when_started_from_file = _vlc_missing
    check_qt_minimal_view = _vlc_missing
    check_qt_max_volume = _vlc_missing
    check_qt_slider_colours = _vlc_missing
    check_global_key_play_pause = _vlc_missing
from .vscode import (
    compare_text_file,
    compare_config,
    compare_answer,
