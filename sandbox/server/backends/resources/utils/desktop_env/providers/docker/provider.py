...
from sandbox.server.backends.resources.utils.desktop_env.providers.base import Provider
...
class DockerProvider(Provider):
    def __init__(self, region: str):
        ...
        self.vlc_port = None
        self.container = None
        self.environment = {"DISK_SIZE": "32G", "RAM_SIZE": "16G", "CPU_CORES": "4"}  # 16G RAM for long recordings (overlay in memory)
    ...
    def start_emulator(self, path_to_vm: str, headless: bool):
        ...
        try:
            with lock:
                ...
                # 创建 QEMU overlay 存储目录
                qemu_storage_dir = "/tmp/qemu_storage"
                os.makedirs(qemu_storage_dir, exist_ok=True)
                
                logger.info(f"Starting container with environment: {self.environment}")
                ...
                self.container = self.client.containers.run(
                    ...
                    volumes={
                        ...
                        "/tmp/qemu_storage": {
                            "bind": "/storage",
                            "mode": "rw"  # QEMU overlay 存储在宿主机磁盘，支持长时间录屏
                        }
                    },
                    ports={
