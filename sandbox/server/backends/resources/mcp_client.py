...
def resolve_mcp_value(
    ...
    *,
    ...
    unsupported = sorted(found_placeholders - _SUPPORTED_PLACEHOLDERS)
    if unsupported:
        supported = ", ".join(sorted(_SUPPORTED_PLACEHOLDERS))
        names = ", ".join(unsupported)
        raise ValueError(
            f"Unsupported placeholder(s): {names}. "
...
def load_mcp_process_config(
    *,
    ...
    config_path = Path(config_dir) if config_dir else toolathlon_root / "configs" / "mcp_servers"
    ...
    local_servers_path = str(toolathlon_root / "local_servers")
